<?php
    ...
    class Blog extends \Wezom\Modules\Ajax {
        ...
        public function getItemAction() {
            ...
            $item = DB::select('blog.id', 'blog.name', 'blog.date', 'blog.image', ['blog_rubrics.name', 'rubric'])
                ->from('blog')
        ...
        public function searchItemsAction() {
            ...
            $result = DB::select('blog.id', 'blog.name', 'blog.image', ['blog_rubrics.name', 'rubric'])
                ->from('blog')
                ->join('blog_rubrics', 'LEFT')->on('blog_rubrics.id', '=', 'blog.rubric_id');
            if( $parent_id > 0 ) {
            ...
            $count = DB::select([DB::expr('COUNT(DISTINCT blog.id)'), 'count'])->from('blog');
            if( $parent_id > 0 ) {
