...
function deleteDir($dir) {
    ...
    foreach (scandir($dir) as $item) {
        ...
        if (!deleteDir($dir.DS.$item)) return false;
    }
    return rmdir($dir);
}
...
function rcopy($source, $destination, $is_rec = FALSE) {
    if (is_dir($source)) {
        if ($is_rec === FALSE){
            ...
            $destination = rtrim($destination, '/').DS.$pinfo['basename'];
        }
        ...
        foreach ($files as $file){
            if ($file != "." && $file != "..") {
                rcopy($source.DS.$file, rtrim($destination, '/').DS.$file, TRUE);
            }
        }
    }
    else {
        if (file_exists($source)){
            if (is_dir($destination) === TRUE){
                $pinfo = pathinfo($source);
                $dest2 = rtrim($destination, '/').DS.$pinfo['basename'];
            }
            else {
                $dest2 = $destination;
            }
...
function rrename($source, $destination, $is_rec = FALSE) {
    if (is_dir($source)) {
        ...
        foreach ($files as $file){
            if ($file != "." && $file != "..") {
                rrename($source.DS.$file, rtrim($destination, '/').DS.$file, TRUE);
            }
        }
    }
    else {
    ...
    foreach ($files as $file) {
        if ($file != "." && $file != "..") {
            if (is_dir($source.DS.$file)){
                rrename_after_cleaner($source.DS.$file);
            }
            else {
        ...
        foreach ($files as $file) 
        {
            if ($file != "." && $file != "..") 
            {
                if (is_dir($source.DS.$file))
                {
                    ...
                    {
                        chmod($source.DS.$file, $mode);
                    }
                    rchmod($source.DS.$file, $mode, $rec_option, TRUE);
                }
                else 
                {
