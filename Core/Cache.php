 ...
 * or by extending `Cache` class) to the name of the group, and use:
 *
...
abstract class Cache
{
    ...
    public static $cache_dir = 'Cache/TMP';
    ...
    public static function instance($group = null)
    {
        ...
        if (!Arr::get($config, $group, false)) {
            die('Failed to load Cache group: ' . $group);
        }
        $config = Arr::get($config, $group, []);
        // Create a new cache type instance
        $cache_class = '\\Core\\Cache\\' . ucfirst($config['driver']);
    ...
    {
        die('Cloning of Cache objects is forbidden');
    }
