...
/**
 * (api/Kohana_Cache) File driver. Provides a file based
 ...
 */
class File extends \Core\Cache implements GarbageCollect
{
    ...
    protected function __construct(array $config)
    {
        ...
        $directory = rtrim(HOST, DS) . DS . Arr::get($this->_config, 'cache_dir', Cache::$cache_dir);

        Files::createFolder($directory, 0777);

        $this->_cache_dir = new \SplFileInfo($directory);

        // If the defined directory is a file, get outta here
        if ($this->_cache_dir->isFile()) {
            die('Unable to create cache directory as a file already exists: ' . $this->_cache_dir->getRealPath());
        }

        // Check the read status of the directory
        if (!$this->_cache_dir->isReadable()) {
            die('Unable to read from the cache directory ' . $this->_cache_dir->getRealPath());
        }

        // Check the write status of the directory
        if (!$this->_cache_dir->isWritable()) {
            die('Unable to write to the cache directory ' . $this->_cache_dir->getRealPath());
        }
    }
    ...
    {
        $filename = File::filename($this->_sanitize_id($id));
        ...
        try {
            ...
            if ($e->getCode() === E_NOTICE) {
                die(__METHOD__ . ' failed to unserialize cached object with message : ' . $e->getMessage());
            }
    ...
    public function set($id, $data, $lifetime = NULL)
    {
        ...
        if (!$dir->isDir()) {
            ...
            if (!mkdir($directory, 0777, TRUE)) {
                die(__METHOD__ . ' unable to create directory : ' . $directory);
            }
        ...
        try {
            ...
            if ($e->getCode() === E_NOTICE) {
                ...
                die(__METHOD__ . ' failed to serialize data for caching with message : ' . $e->getMessage());
            }
            ...
            if ($file->isFile()) {
                try {
                    ...
                    if ($e->getCode() === E_WARNING) {
                        die(__METHOD__ . ' failed to delete file : ' . $file->getRealPath());
                    }
                }
            } // Else, is directory
            elseif ($file->isDir()) {
                ...
                try {
                    ...
                    if ($e->getCode() === E_WARNING) {
                        die(__METHOD__ . ' failed to delete directory : ' . $file->getRealPath());
                    }
                    throw $e;
                }
            } else {
    ...
    protected function _make_directory($directory, $mode = 0777, $recursive = FALSE, $context = NULL)
    {
        if (!mkdir($directory, $mode, $recursive, $context)) {
            die('Failed to create the defined cache directory : ' . $directory);
        }
