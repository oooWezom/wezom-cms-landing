        ...
        if ($var === null) {
            ...
            if (($type = get_resource_type($var)) === 'stream' and $meta = stream_get_meta_data($var)) {
                ...
                if (isset($meta['uri'])) {
                    ...
                    return '<small>resource</small><span>(' . $type . ')</span> ' . htmlspecialchars($file, ENT_NOQUOTES, static::$charset);
                }
            } else {
            ...
            $var = UTF8::clean($var, static::$charset);

            if (UTF8::strlen($var) > $length) {
                // Encode the truncated string
                $str = htmlspecialchars(substr($var, 0, $length), ENT_NOQUOTES, static::$charset) . '&nbsp;&hellip;';
            } else {
                // Encode the string
                $str = htmlspecialchars($var, ENT_NOQUOTES, static::$charset);
            }
            ...
            if (empty($var)) {
                ...
                foreach ($var as $key => & $val) {
                    ...
                    if (!is_int($key)) {
                        $key = '"' . htmlspecialchars($key, ENT_NOQUOTES, static::$charset) . '"';
                    }
            ...
            return '<small>' . gettype($var) . '</small> ' . htmlspecialchars(print_r($var, TRUE), ENT_NOQUOTES, static::$charset);
        }
    }
    ...
    public static function source($file, $line_number, $padding = 5)
    {
        ...
        while (($row = fgets($file)) !== false) {
            ...
            if ($line >= $range['start']) {
                ...
                $row = htmlspecialchars($row, ENT_NOQUOTES, static::$charset);
