...
class PHPMailer
{
    ...
    protected function addAnAddress($kind, $address, $name = '')
    {
        if (!preg_match('/^(to|cc|bcc|Reply-To)$/', $kind)) {
            ...
            if ($this->exceptions) {
                throw new MException('Invalid recipient array: ' . $kind);
            }
            return false;
        }
        ...
        if (!$this->validateAddress($address)) {
            ...
            if ($this->exceptions) {
                throw new MException($this->lang('invalid_address') . ': ' . $address);
            }
            return false;
        }
        if ($kind != 'Reply-To') {
        ...
        if ($this->isError()) {
            ...
            try {
                if (!defined('PKCS7_TEXT')) {
                    throw new MException($this->lang('signing') . ' OpenSSL extension missing.');
                }
    ...
    public function set($name, $value = '')
    {
        try {
            if (isset($this->$name)) {
                ...
                throw new MException($this->lang('variable_set') . $name, self::STOP_CRITICAL);
            }
        } catch (Exception $e) {
            $this->setError($e->getMessage());
