 ...
 */
class Memcache extends \Core\Cache implements Arithmetic
{
    ...
    protected function __construct(array $config)
    {
        ...
        if (!$servers) {
            ...
            die('No Memcache servers defined in configuration');
        }
        ...
        foreach ($servers as $server) {
            ...
            if (!$this->_memcache->addServer($server['host'], $server['port'], $server['persistent'], $server['weight'], $server['timeout'], $server['retry_interval'], $server['status'], $server['failure_callback'])) {
                die('Memcache could not connect to host ' . $server['host'] . ' using port ' . $server['port']);
            }
        }
    ...
    public function set($id, $data, $lifetime = 3600)
    {
        ...
        if ($lifetime > Memcache::CACHE_CEILING) {
            // Set the lifetime to maximum cache time
            $lifetime = Memcache::CACHE_CEILING + time();
