<?php
    ...
    class Filter {
        ...
        static $_items = []; // Array of all items in group
        static $_data = [];
        static $_ids = [0];
        static $_tmp = [];
        ...
        static function getFilteredItems() {
            if(!self::$_items) {
                self::$_tmp[] = DB::select(
                    ...
                    [DB::expr('NULL'), 'model_alias'],
                    [DB::expr('NULL'), 'specification_alias'],
                    [DB::expr('NULL'), 'specification_value_alias']
                )
                    ->from('catalog')
                ...
                self::$_tmp[] = DB::select(
                    'catalog.id', 'catalog.available', 'catalog.cost',
                    [DB::expr('NULL'), 'brand_alias'],
                    ['models.alias', 'model_alias'],
                    [DB::expr('NULL'), 'specification_alias'],
                    [DB::expr('NULL'), 'specification_value_alias']
                )
                    ->from('catalog')
                ...
                self::$_tmp[] = DB::select(
                    ...
                    ['specifications_values.alias', 'specification_value_alias']
                )
                    ->from('catalog')
        ...
        public static function getFilteredItemsList( $limit = 15, $offset = 0, $sort = 'sort', $type = 'DESC' ) {
            ...
            $list = [];
            $filter = is_array(Config::get('filter_array')) ? Config::get('filter_array') : [];
            ...
            $result = DB::select('catalog.*', ['catalog_images.image', 'image'], ['brands.name', 'brand_name'], ['brands.alias', 'brand_alias'])
                    ->from('catalog')
