...
class Rules
{
    ...
    {
        if (Route::param('id')) {
            $res = DB::select([DB::expr('COUNT(id)'), 'count'])->from($table)->where($field, '=', $str)->where('id', '!=', Route::param('id'))->count_all();
        } else {
            $res = DB::select([DB::expr('COUNT(id)'), 'count'])->from($table)->where($field, '=', $str)->count_all();
        }
        if ($res) {
            return false;
        }
